...
class AssetLoaderMixin(object):
    """Contains logic to load assets from given texture paths in order of precence
    #optimize: much of the logic in here should be combined to reduce codebase
    """
    ...
    def load_file(self, path:str, name:str, ext:str):
        if ":" in name:
            return self.find_file("{0}/{1}{2}".format(path,name.split(":")[1],ext), verbose=False)

        else:
            return self.find_file("{0}/{1}{2}".format(path,name,ext), verbose=False)
